...
- `alignment` (Hash) - Horizontal and/or Vertical center. Example:  `alignment: {horizontal: true, vertical: true}`
...
- `align` (Boolean) - Similar to `alignment`. Example `align: true` is the same as `alignment: {horizontal: true, vertical: false}`
